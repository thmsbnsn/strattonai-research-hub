...
import { paperTrades, portfolioPerformance } from "@/lib/mockData";
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from "recharts";
import { ArrowUpRight, ArrowDownRight } from "lucide-react";

export default function PaperTrades() {
  const totalPnl = paperTrades
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="terminal-card p-4 text-center">
            ...
            <p className={`font-mono text-2xl font-bold ${totalPnl >= 0 ? "text-success" : "text-danger"}`}>
              {totalPnl >= 0 ? "+" : ""}${totalPnl.toFixed(0)}
            </p>
          </div>
          <div className="terminal-card p-4 text-center">
            <p className="text-xs text-muted-foreground mb-1">Win Rate</p>
            <p className="font-mono text-2xl font-bold text-foreground">67%</p>
          </div>
          <div className="terminal-card p-4 text-center">
            <p className="text-xs text-muted-foreground mb-1">Closed Trades</p>
            <p className="font-mono text-2xl font-bold text-foreground">
              {paperTrades.filter((t) => t.status === "Closed").length}
            </p>
          </div>
        </div>
        ...
        <div className="terminal-card p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">Portfolio Performance (30 Days)</h3>
          <div className="h-48">
